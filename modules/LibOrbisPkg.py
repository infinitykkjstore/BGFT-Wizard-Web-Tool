...
Uso:
    ...
    python give_meta.py --url https://cdn.com/manifest.json --output metadata.json
"""
...
class ManifestResolver:
    """
    Resolve ranges de um manifesto de múltiplas partes.
    Suporta manifestos no formato:
    {
      "originalFileSize": 16869097472,
      "packageDigest": "...",
      "numberOfSplitFiles": 9,
      "pieces": [
        {"url": "...", "fileOffset": 0, "fileSize": 2040109465, "hashValue": "..."},
        ...
      ]
    }
    """
    
    def __init__(self, manifest_url: str, session: requests.Session, verbose: bool = False):
    ...
    def _load_manifest(self):
        """Baixa e parseia o manifesto"""
        self.log("Baixando manifesto...")
        
        try:
            response = self.session.get(self.manifest_url, timeout=30, allow_redirects=True)
            if response.status_code != 200:
                raise Exception(f"HTTP {response.status_code} ao baixar manifesto")
            
            data = response.json()
            
            self.original_size = data.get('originalFileSize', 0)
            self.package_digest = data.get('packageDigest', '')
            self.pieces = data.get('pieces', [])
            
            self.log(f"Manifesto: {len(self.pieces)} parts, {self.original_size} bytes", "INFO")
            
            if not self.pieces:
                raise Exception("Manifesto sem pieces")
                
        except json.JSONDecodeError as e:
            raise Exception(f"Manifesto JSON inválido: {e}")
    ...
    def get_pieces_covering_range(self, start: int, end: int) -> List[Dict[str, Any]]:
        """Retorna as pieces que cobrem o range [start, end]"""
        covering = []
        for piece in self.pieces:
            piece_start = piece.get('fileOffset', 0)
            piece_end = piece_start + piece.get('fileSize', 0) - 1
            
            if start <= piece_end and end >= piece_start:
                covering.append(piece)
        
        return covering
    
    def fetch_range(self, start: int, end: int) -> bytes:
        """
        Faz HTTP range request resolvendo para as pieces apropriaadas.
        Preserva 302/307 redirects com range.
        """
        pieces = self.get_pieces_covering_range(start, end)
        
        if not pieces:
            raise Exception(f"Nenhuma piece cobre o range {start}-{end}")
        
        if len(pieces) == 1:
            return self._fetch_single_piece(pieces[0], start, end)
        
        result = bytearray()
        for piece in pieces:
            data = self._fetch_single_piece(piece, start, end)
            result.extend(data)
        
        return bytes(result)
    
    def _fetch_single_piece(self, piece: Dict[str, Any], global_start: int, global_end: int) -> bytes:
        """Faz request para uma única piece"""
        url = piece['url']
        piece_offset = piece['fileOffset']
        piece_size = piece['fileSize']
        
        piece_start = piece_offset
        piece_end = piece_offset + piece_size - 1
        
        req_start = max(global_start - piece_start, 0)
        req_end = min(global_end, piece_end) - piece_start
        
        if req_start >= piece_size:
            return b''
        
        self.log(f"Fetching piece {url.split('/')[-1]} bytes {req_start}-{req_end}", "DEBUG")
...
class PKGMetadataExtractor:
    ...
    Suporta tanto URLs diretas de PKG quanto manifestos JSON.
    """
    ...
    def __init__(self, url: str, verbose: bool = False, range_reader: Optional[Callable[[int, int], bytes]] = None):
        self.url = url
        self.verbose = verbose
    ...
    def _detect_input_type(self):
        """Detecta se a URL é um manifesto JSON ou PKG direto"""
        url_lower = self.url.lower()
        
        if url_lower.endswith('.json') or url_lower.endswith('.manifest'):
            self._try_load_manifest()
            return
        
        if url_lower.startswith('http'):
            try:
                head = self.session.head(self.url, timeout=10, allow_redirects=True)
        ...
        self.log("URL detectada como PKG direto", "DEBUG")
    
    def _try_load_manifest(self):
        """Tenta carregar como manifesto"""
        try:
            self.log("Detectado como manifesto, tentanto carregar...", "INFO")
            self.manifest_resolver = ManifestResolver(self.url, self.session, self.verbose)
            self.is_manifest = True
            self.log("Manifesto carregado com sucesso!", "SUCCESS")
        except Exception as e:
            self.log(f"Não é manifesto válido: {e}", "DEBUG")
            self.is_manifest = False
    ...
    def fetch_range(self, start: int, end: int, retry: int = 0) -> bytes:
        ...
        if self.is_manifest and self.manifest_resolver:
            return self.manifest_resolver.fetch_range(start, end)
        
        if self.range_reader is not None:
            data = self.range_reader(start, end)
            if data is None:
                raise Exception("Custom range reader returned no data")
            return data
